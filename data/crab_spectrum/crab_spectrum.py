...
# From http://arxiv.org/pdf/astro-ph/9710211.pdf

Ecutoff_1Mev = 41  # MeV
N0_1Mev = 1.29e-4  # pht cm^-2 s^-1 MeV-1
gamma_1Mev = 2.02
energies_1Mev = 10.0 ** np.arange(0.1, 2, 0.01)
...
#plt.loglog(mpt_2kev*1000, ((dn_de_2kev[1:] + dn_de_2kev[:-1]) / 2.0)*(energies_2kev[1:] - energies_2kev[:-1]), '-+g')
plt.loglog(mpt_2kev*1000, ((dn_de_2kev[1:] + dn_de_2kev[:-1]) / 2.0), '-+g', label='TTM')

#plt.loglog(mpt0_int*1000, ((dn_de0_int[1:] + dn_de0_int[:-1]) / 2.0)*(energies0_int[1:] - energies0_int[:-1]), '-+r')
#plt.loglog(mpt1_int*1000, ((dn_de1_int[1:] + dn_de1_int[:-1]) / 2.0)*(energies1_int[1:] - energies1_int[:-1]), '-+r')
plt.loglog(mpt0_int*1000, ((dn_de0_int[1:] + dn_de0_int[:-1]) / 2.0), '-+r', label='Integral')
plt.loglog(mpt1_int*1000, ((dn_de1_int[1:] + dn_de1_int[:-1]) / 2.0), '-+r')

plt.loglog(mpt_1Mev*1e6, ((dn_de_1Mev[1:] + dn_de_1Mev[:-1]) / 2.0)/1000.0, '-+c', label='COMPTEL')

plt.loglog(mpt_100Mev, ((dn_de_100Mev[1:] + dn_de_100Mev[:-1]) / 2.0)/1000.0, '-+b', label='FERMI LAT')

plt.xlabel('$X/\gamma$-ray energy ($eV$)')
plt.ylabel('Flux ($photon/s/cm^{2}/keV$)') 
print 'Total integrated flux (2-30kev)', np.sum(((dn_de_2kev[1:] + dn_de_2kev[:-1]) / 2.0)*(energies_2kev[1:] - energies_2kev[:-1])),'photons/s/cm^2'
print 'Total integrated flux (100-111000Mev)', np.sum(((dn_de_100Mev[1:] + dn_de_100Mev[:-1]) / 2.0)*(energies_100Mev[1:] - energies_100Mev[:-1])/1e6),'photons/s/cm^2'
